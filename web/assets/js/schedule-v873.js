...
(function() {
    ...
    PiSignage.Schedule = {
        ...
        createScheduleCard: function(schedule) {
            ...
            const playlistExists = this.playlists.some(p => p.name === schedule.playlist);

            const card = document.createElement('div');
            card.className = 'schedule-item';
            if (!playlistExists) {
                card.classList.add('schedule-warning'); // Classe CSS pour warning visuel
            }
                ...
                <div class="schedule-content">
                    <div class="schedule-header">
                        <div class="schedule-title-section">
                            ...
                            <span class="schedule-playlist ${!playlistExists ? 'text-danger' : ''}">
                                Playlist: "${this.escapeHtml(schedule.playlist)}"
                                ${!playlistExists ? '<strong> (INTROUVABLE)</strong>' : ''}
                            </span>
                            ${schedule.description ? `<p class="schedule-description">${this.escapeHtml(schedule.description)}</p>` : ''}
                        </div>

                        <div class="schedule-toggle">
                            <label class="toggle-switch" ${!playlistExists ? 'title="Impossible d\'activer - Playlist manquante"' : ''}>
                                <input type="checkbox" ${schedule.enabled ? 'checked' : ''}
                                    ${!playlistExists ? 'disabled' : ''}
        ...
        validateFormData: function(data) {
            ...
            if (!data.playlist) {
                ...
                const playlistExists = this.playlists.some(p => p.name === data.playlist);
                if (!playlistExists) {
                    errors.push(`La playlist "${data.playlist}" n'existe pas ou a été supprimée. Veuillez en sélectionner une autre.`);
                }
            }
        ...
        toggleSchedule: async function(scheduleId) {
            ...
            try {
                // Construction explicite avec validation
                const url = `/api/schedule.php/${scheduleId}/toggle`;
                console.log('[Schedule] Toggle URL:', url);
        ...
        deleteSchedule: async function(scheduleId) {
            console.log('[Schedule] deleteSchedule called with:', scheduleId);
            console.trace('[Schedule] Call stack trace');

            // Prevent concurrent deletes
            if (this._isDeleting) {
                console.warn('[Schedule] Delete already in progress, ignoring duplicate call');
                return;
            }
            ...
            if (!confirm(`Supprimer le planning "${schedule.name}" ?\n\nCette action est irréversible.`)) {
                console.log('[Schedule] Delete cancelled by user');
                return;
            }
            ...
            try {
                ...
                const url = `/api/schedule.php/${scheduleId}?t=${Date.now()}`;
                console.log('[Schedule] Delete URL:', url);
                console.log('[Schedule] scheduleId value:', scheduleId);
                console.log('[Schedule] typeof scheduleId:', typeof scheduleId);
